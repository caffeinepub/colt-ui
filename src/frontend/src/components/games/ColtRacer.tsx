  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      ...
      if (stateRef.current === "playing" && keyDebounceRef.current === 0) {
        if (
          ...
          playerXRef.current =
            playerLaneRef.current * LANE_W + LANE_W / 2 - PLAYER_W / 2;
          keyDebounceRef.current = 12;
        } else if (
          (e.code === "ArrowRight" || e.code === "KeyD") &&
          playerLaneRef.current < LANE_COUNT - 1
        ) {
          playerLaneRef.current++;
          playerXRef.current =
            playerLaneRef.current * LANE_W + LANE_W / 2 - PLAYER_W / 2;
    ...
    const loop = () => {
      ...
      roadOffsetRef.current =
        (roadOffsetRef.current +
          (stateRef.current === "playing" ? speedRef.current : 0)) %
        60;
      ...
      if (stateRef.current === "playing") {
        ...
        const spawnInterval = Math.max(
          40,
          90 - Math.floor(scoreRef.current / 50) * 5,
        );
        if (frameRef.current % spawnInterval === 0) {
          const usedLanes = obstaclesRef.current
            .filter((o) => o.y < 80)
            .map((o) => o.lane);
      ...
      if (stateRef.current === "playing") {
        ...
        ctx.fillRect(
          playerAnimXRef.current + 8,
          playerY + PLAYER_H,
          4,
          speedRef.current * 3,
        );
        ctx.fillRect(
          playerAnimXRef.current + PLAYER_W - 12,
          playerY + PLAYER_H,
          4,
          speedRef.current * 3,
        );
      ...
      ctx.shadowBlur = 8;
      ctx.fillText(
        `SPEED: ${(speedRef.current * 25).toFixed(0)}km/h`,
        W - 10,
        25,
      );
      ctx.shadowBlur = 0;
      ...
      if (stateRef.current === "idle") {
        ...
        ctx.fillText(
          "← → ARROW KEYS or CLICK to change lanes",
          W / 2,
          H / 2 + 35,
        );
      }
    ...
    <div className="flex flex-col items-center gap-4">
      <div className="flex items-center gap-6 font-display text-sm tracking-wider">
        <span className="text-muted-foreground">
          DIST: <span className="text-neon-cyan">{displayScore}m</span>
        </span>
        <span className="text-muted-foreground">
          BEST: <span className="text-neon-green">{bestScore}m</span>
        </span>
      </div>
      <div
        className="relative"
        ...
        {gameState === "dead" && (
          ...
          >
            ...
            <p className="font-body text-muted-foreground mb-1">
              Distance:{" "}
              <span className="text-neon-cyan font-bold">{displayScore}m</span>
            </p>
            <p className="font-body text-muted-foreground mb-6">
              Best:{" "}
              <span className="text-neon-green font-bold">{bestScore}m</span>
            </p>
            <button
              type="button"
              onClick={() => {
