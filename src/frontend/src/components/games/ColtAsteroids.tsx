...
const ColtAsteroids: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (stateRef.current === "playing") {
        ...
        if (keysRef.current.has("Space") && shootCooldownRef.current === 0) {
          ...
          bulletsRef.current.push({
            ...
            vel: {
              x: Math.cos(ship.angle) * speed + ship.vel.x,
              y: Math.sin(ship.angle) * speed + ship.vel.y,
            },
            life: 55,
          });
              ...
              if (a.radius > 18) {
                for (let j = 0; j < 2; j++) {
                  ...
                  newAsteroids.push({
                    ...
                    vel: {
                      x: Math.cos(angle) * Math.abs(speed) + 0.5,
                      y: Math.sin(angle) * Math.abs(speed) + 0.5,
                    },
                    radius: newR,
                    angle: 0,
        ...
        if (asteroidsRef.current.length === 0) {
          ...
          asteroidsRef.current = Array.from({ length: count }, () =>
            spawnAsteroid(levelRef.current),
          );
        }
      ...
      // Draw asteroids
      for (const ast of asteroidsRef.current) drawAsteroid(ast);
      ...
      if (stateRef.current === "playing") {
        const alpha =
          invincibleRef.current > 0
            ? Math.floor(frameRef.current / 6) % 2 === 0
              ? 0.3
              : 1
            : 1;
    ...
    <div className="flex flex-col items-center gap-4">
      <div className="flex items-center gap-6 font-display text-sm tracking-wider">
        <span className="text-muted-foreground">
          SCORE: <span style={{ color: "#ff8800" }}>{displayScore}</span>
        </span>
        <span className="text-muted-foreground">
          LIVES:{" "}
        ...
        {gameState === "dead" && (
          ...
          >
            ...
            <p className="font-body text-muted-foreground mb-1">
              Score:{" "}
              <span style={{ color: "#ff8800" }} className="font-bold">
                {displayScore}
              </span>
            </p>
