...
const SnakeGame: React.FC<{ difficulty?: Difficulty }> = ({
  difficulty = "normal",
}) => {
  const tickSpeed =
    difficulty === "easy" ? 150 : difficulty === "hard" ? 60 : 100;
    ...
    <div className="flex flex-col items-center gap-4">
      <div className="flex items-center gap-6 font-display text-sm tracking-wider">
        <span className="text-muted-foreground">
          SCORE: <span className="text-neon-green">{displayScore}</span>
        </span>
        <span className="text-muted-foreground">
          BEST: <span className="text-neon-cyan">{bestScore}</span>
        </span>
      </div>
        ...
        {gameState === "dead" && (
          ...
          >
            ...
            <p className="font-body text-muted-foreground mb-1">
              Score:{" "}
              <span className="text-neon-green font-bold">{displayScore}</span>
            </p>
            <p className="font-body text-muted-foreground mb-6">
              Best:{" "}
              <span className="text-neon-cyan font-bold">{bestScore}</span>
            </p>
            <button
              type="button"
              onClick={startGame}
