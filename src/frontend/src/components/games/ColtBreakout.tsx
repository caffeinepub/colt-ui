    ...
    const loop = () => {
      ...
      if (stateRef.current === "playing") {
        ...
        paddleXRef.current = Math.max(
          0,
          Math.min(W - PADDLE_W, paddleXRef.current),
        );

        if (keysRef.current.has("ArrowLeft"))
          paddleXRef.current = Math.max(0, paddleXRef.current - 7);
        if (keysRef.current.has("ArrowRight"))
          paddleXRef.current = Math.min(W - PADDLE_W, paddleXRef.current + 7);
        ...
        if (ballAttachedRef.current) {
          ...
          // Wall bounces
          if (ballXRef.current - BALL_R < 0) {
            ballXRef.current = BALL_R;
            ballVxRef.current = Math.abs(ballVxRef.current);
          }
          if (ballXRef.current + BALL_R > W) {
            ballXRef.current = W - BALL_R;
            ballVxRef.current = -Math.abs(ballVxRef.current);
          }
          if (ballYRef.current - BALL_R < 0) {
            ballYRef.current = BALL_R;
          ...
          if (
            ...
            const hitPos =
              (ballXRef.current - paddleXRef.current) / PADDLE_W - 0.5;
            const speed = Math.sqrt(
              ballVxRef.current ** 2 + ballVyRef.current ** 2,
            );
          ...
          for (const brick of bricksRef.current) {
            ...
            if (
              ...
              const overlapRight =
                brick.x + BRICK_W - BRICK_PAD - (ballXRef.current - BALL_R);
              const overlapTop = ballYRef.current + BALL_R - brick.y;
              const overlapBottom =
                brick.y + BRICK_H - (ballYRef.current - BALL_R);
      ...
      ctx.shadowBlur = 8;
      ctx.fillText(
        `LIVES: ${"♥".repeat(Math.max(0, livesRef.current))}`,
        W - 10,
        30,
      );
      ctx.shadowBlur = 0;
      ...
      if (stateRef.current === "idle") {
        ...
        ctx.fillText(
          "MOUSE / ARROW KEYS to move · SPACE to launch",
          W / 2,
          H / 2 + 35,
        );
      }
    ...
    <div className="flex flex-col items-center gap-4">
      <div className="flex items-center gap-6 font-display text-sm tracking-wider">
        <span className="text-muted-foreground">
          SCORE: <span style={{ color: "#ffcc00" }}>{displayScore}</span>
        </span>
        <span className="text-muted-foreground">
          LIVES:{" "}
          <span style={{ color: "#ff0055" }}>
            {"♥".repeat(Math.max(0, displayLives))}
          </span>
        </span>
        <span className="text-muted-foreground">
          BEST: <span className="text-neon-green">{bestScore}</span>
        </span>
      </div>
      <div
        className="relative"
        ...
        {gameState === "dead" && (
          ...
          >
            <p
              ...
              style={{ color: "#ff0055", textShadow: "0 0 20px #ff0055" }}
            >
              GAME OVER
            </p>
            ...
            <p className="font-body text-muted-foreground mb-6">
              Best:{" "}
              <span className="text-neon-green font-bold">{bestScore}</span>
            </p>
            <button
              type="button"
              onClick={() => {
