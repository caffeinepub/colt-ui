...
const ColtPong: React.FC = () => {
  ...
  const [_gameState, setGameState] = useState<GameState>("idle");
      ...
      if (stateRef.current === "playing") {
        ...
        if (keysRef.current.has("ArrowDown") || keysRef.current.has("KeyS")) {
          playerYRef.current = Math.min(
            H - PADDLE_H,
            playerYRef.current + PADDLE_SPEED,
          );
        }
        // Mouse control
        ...
        if (Math.abs(diff) > 1) {
          playerYRef.current +=
            Math.sign(diff) * Math.min(Math.abs(diff), PADDLE_SPEED * 1.5);
          playerYRef.current = Math.max(
            0,
            Math.min(H - PADDLE_H, playerYRef.current),
          );
        }
        ...
        if (
          ...
          const hitPos =
            (ballYRef.current - playerYRef.current) / PADDLE_H - 0.5;
          const speed =
            Math.sqrt(ballVxRef.current ** 2 + ballVyRef.current ** 2) + 0.15;
        ...
        if (
          ...
          const speed =
            Math.sqrt(ballVxRef.current ** 2 + ballVyRef.current ** 2) + 0.1;
          ballVxRef.current = -Math.abs(speed * Math.cos(hitPos * 1.2));
          ballVyRef.current = speed * Math.sin(hitPos * 1.2) * 2;
        }
    ...
    <div className="flex flex-col items-center gap-4">
      <div className="flex items-center gap-8 font-display text-sm tracking-wider">
        <span className="text-muted-foreground">
          YOU: <span className="text-neon-cyan">{displayPlayerScore}</span>
        </span>
        <span className="text-muted-foreground">
          CPU: <span style={{ color: "#ff00aa" }}>{displayCpuScore}</span>
        </span>
      </div>
      <div
        className="relative"
