...
const FlappyColt: React.FC<{ difficulty?: Difficulty }> = ({
  difficulty = "normal",
}) => {
  const gravityVal =
    difficulty === "easy" ? 0.25 : difficulty === "hard" ? 0.5 : 0.35;
  const pipeSpeedVal =
    difficulty === "easy" ? 2 : difficulty === "hard" ? 3.5 : 2.5;
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (stateRef.current === "playing") {
        ...
        // Physics
        birdVelRef.current += gravityVal;
        birdYRef.current += birdVelRef.current;
        birdRotRef.current = Math.max(
          -0.5,
          Math.min(1.2, birdVelRef.current * 0.06),
        );
    ...
    <div className="flex flex-col items-center gap-4">
      ...
      >
        ...
        {gameState === "dead" && (
          ...
          >
            <p
              className="font-display text-2xl text-destructive tracking-widest mb-2"
            ...
            <p className="font-body text-muted-foreground mb-1">
              Score:{" "}
              <span className="text-neon-cyan font-bold">{displayScore}</span>
            </p>
