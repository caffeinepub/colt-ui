...
export default function StoreTab() {
  ...
  const handleBuy = async (item: StoreItem) => {
    ...
    const success = await spendCCoins.mutateAsync(BigInt(0));
    if (success) {
    ...
    <div className="flex flex-col gap-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {STORE_ITEMS.map((item) => {
          ...
          const isBuying =
            spendCCoins.isPending || setPurchasedEffects.isPending;
              ...
              <div className="mt-auto flex items-center justify-between">
                ...
                {owned ? (
                  ...
                  <button
                    ...
                    className="text-xs px-3 py-1.5 rounded-lg font-semibold transition-all bg-yellow-500/20 border border-yellow-500/50 text-yellow-400 hover:bg-yellow-500/30 hover:scale-105 disabled:opacity-50"
                  >
                    {isBuying ? (
      ...
      <p className="text-center text-xs text-gray-600">
        💡 All effects are free — unlock them all!
      </p>
    </div>
  );
}
