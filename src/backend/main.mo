...
actor {
  ...
  // Map from users to user preferences
  let preferencesMap = Map.empty<Principal, Preferences>();
  // User profiles
  let userProfiles = Map.empty<Principal, UserProfile>();
  // Stores user C Coins balances
  let cCoinsBalances = Map.empty<Principal, Nat>();
  // Stores user-purchased visual effects
  let purchasedEffectsMap = Map.empty<Principal, Set.Set<Text>>();
  ...
  // Save notepad content as part of the preferences.
  public shared ({ caller }) func saveNotepad(notepad : Text) : async () {
    ...
    switch (preferencesMap.get(caller)) {
      ...
      case (null) {
        ...
        preferencesMap.add(caller, defaultPrefs);
      };
    };
  };
  ...
  // No authorization check: anonymous principal and all callers are valid for addCCoins.
  public shared ({ caller }) func addCCoins(amount : Nat) : async () {
    switch (cCoinsBalances.get(caller)) {
      case (null) {
        let newBalance = 2000 + amount; // Initialize new users/anonymous principal with 2,000 C Coins.
        cCoinsBalances.add(caller, newBalance);
      };
      case (?balance) {
        let newBalance = balance + amount;
